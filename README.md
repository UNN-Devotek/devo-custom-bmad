...
The `output/` directory shows exactly what each package writes to the target project after installation:

```
output/
├── agent-orchestration/    # Files installed to {project-root}/_devo-bmad-custom/
│   ├── core/               # Agents, config, workflows
│   ├── bmm/                # Module agents
│   ├── bmb/                # Builder agents
│   ├── _memory/            # Sidecar instructions
│   └── .claude/commands/   # bmad-track-*.md slash commands (persona-scrubbed)
├── squidhub-addon/         # Additional files merged into {project-root}/_devo-bmad-custom/
│   ├── squidhub/           # Squidhub-specific agents
│   ├── _memory/squid-master-sidecar/
│   └── .claude/commands/   # bmad-track-*.md slash commands (raw squid-master persona)
└── tmux-config/            # Files installed to ~/.config/tmux/bin/ and ~/.tmux.conf
    ├── tmux.conf
    ├── colors.conf
    └── *.sh                # Tmux helper scripts
```
